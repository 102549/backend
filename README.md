...
🧠 项目开发过程笔记
项目实现思路
分析现有项目: 项目包含 uni-app 前端、Express 网关、文件数据库
创建独立后端: 创建 backend/ 目录，实现完整的 REST API
保留多流支持: 延续原有的多直播流架构，每个流独立管理数据
实现 WebSocket: 添加实时通信功能，支持票数实时推送
接口设计遵循
...
📦 部署说明
后端部署 (Railway)
创建 Railway 账号: https://railway.app
连接 GitHub 仓库
部署 backend 目录
自动获取访问地址
Railway 会自动：
...
在 https://unicloud.dcloud.net.cn 创建项目
使用 HBuilderX 上传前端代码
配置 API 接口地址为后端部署地址
网关部署
...
配置正确的跨域策略
⚠️ 注意事项
跨域配置: 后端已配置 origin: '*'，允许所有来源访问
Mock 模式: 所有数据均为模拟数据，不连接真实数据库
WebSocket: 需要使用 WSS (WSS://) 用于生产环境
端口: 后端默认使用 3000 端口，可通过环境变量 PORT 修改
🔧 本地开发
# 进入后端目录
cd backend
